...
public class BinaryProvenanceReducer extends
    ...
    private static MapWritable provenances = new MapWritable();
    ...
    protected void reduce(RuleWritable key, Iterable<RuleInfoWritable> values,
        ...
        for (RuleInfoWritable ruleInfoWritable: values) {
            for (Writable provenance: ruleInfoWritable.getBinaryProvenance()
                    .keySet()) {
                featureIndex.set(
                        featureStartIndex + ((IntWritable) provenance).get());
                // TODO check that featureIndex is not overwritten
                provenances.put(featureIndex, NullWritable.get());
            }
        }
        context.write(key, provenances);
    }
}
