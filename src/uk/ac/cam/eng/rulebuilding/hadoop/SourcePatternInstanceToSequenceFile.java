...
/**
 * @author jmp84 This class creates source pattern instances
 * from a test file and dumps them to a SequenceFile for further
 * processing.
 */
public class SourcePatternInstanceToSequenceFile {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        SequenceFile.Writer writer = new SequenceFile.Writer(fs, conf, path,
                IntWritable.class, RuleWritable.class);
        int uselessNumber = 0;
        IntWritable uselessKey = new IntWritable();
        for (Rule sourcePatternInstance: sourcePatternInstances) {
        	uselessNumber++;
        	uselessKey.set(uselessNumber);
        	RuleWritable sourcePatternInstanceWritable =
        			RuleWritable.makeSourceMarginal(sourcePatternInstance, true);
        	writer.append(uselessKey, sourcePatternInstanceWritable);
        }
        writer.close();
	}
}
